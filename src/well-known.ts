...
export const WELL_KNOWN_INDEX_VERSION = 1 as const;
...
export function planWellKnownIndexRequest(
  url: string,
  sourcePolicy: SourceSecurityPolicyInput,
): WellKnownIndexRequestPlan {
  ...
  const data = {
    kind: "well-known-index-request" as const,
...
function defaultPort(): WellKnownHttpPort {
  return {
...
export async function fetchWellKnownIndex(
  plan: WellKnownIndexRequestPlan,
  expectedPlanId: string,
  options: WellKnownFetchOptions = {},
): Promise<WellKnownIndexReview> {
  ...
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 60_000) throw new Error("Invalid index timeout");
  ...
  if (!response.ok) throw new Error(`Well-known index request failed with HTTP ${response.status}`);
  const declaredLength = Number(response.headers.get("content-length"));
  if (Number.isFinite(declaredLength) && declaredLength > maximumBytes) {
    throw new Error(`Well-known index exceeds ${maximumBytes} bytes`);
  }
  const text = await response.text();
  if (Buffer.byteLength(text) > maximumBytes) throw new Error(`Well-known index exceeds ${maximumBytes} bytes`);
  const index = wellKnownIndexSchema.parse(JSON.parse(text));
  const seen = new Set<string>();
  const libraries = [...index.libraries]
    .sort((left, right) => left.id.localeCompare(right.id, "en"))
    .map((entry): ReviewedWellKnownLibrary => {
      if (seen.has(entry.id)) throw new Error(`Duplicate well-known library id: ${entry.id}`);
      seen.add(entry.id);
  ...
  const data = {
    kind: "well-known-index-review" as const,
    schemaVersion: 1 as const,
