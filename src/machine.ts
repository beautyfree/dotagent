...
export class NodeMachinePort implements MachinePort {
  readonly #platform: Platform;
  readonly #env: NodeJS.ProcessEnv;

  constructor(options: NodeMachinePortOptions = {}) {
    this.#platform = options.platform ?? (process.platform as Platform);
  ...
  async commandExists(command: string): Promise<boolean> {
    ...
    if (pathApi.isAbsolute(command) || command.includes("/") || command.includes("\\")) {
      ...
      const extensions =
        this.#platform === "win32" ? (this.#env.PATHEXT ?? ".COM;.EXE;.BAT;.CMD").split(";").filter(Boolean) : [""];
      for (const directory of directories) {
        for (const extension of extensions) candidates.push(pathApi.join(directory, `${command}${extension}`));
      }
    }
  ...
  async pathKind(filePath: string): Promise<MachinePathKind> {
    try {
...
export function expandMachinePath(template: string, home: string, platform: Platform): string {
  const pathApi = platform === "win32" ? path.win32 : path.posix;
  if (template === "~") return home;
  if (template.startsWith("~/") || template.startsWith("~\\")) return pathApi.join(home, template.slice(2));
  return template;
}

function deliveryRoots(descriptor: AgentDescriptor, home: string, platform: Platform): string[] {
  return descriptor.skills.flatMap((delivery) =>
    "roots" in delivery ? delivery.roots.map((root) => expandMachinePath(root, home, platform)) : [],
  );
}

async function containsOnlySkillBranches(
  directory: string,
  paths: string[][],
...
export async function isSkillsOnlyMarker(
  marker: string,
  roots: string[],
  platform: Platform,
  port: MachinePort,
...
export async function scanMachineAgents(
  ...
  for (const descriptor of [...descriptors].sort((left, right) => left.slug.localeCompare(right.slug, "en"))) {
    ...
    let result: MachineAgentInventory | null = null;
    for (const rule of descriptor.detection) {
      ...
      if (
        ...
        (await isSkillsOnlyMarker(
          marker,
          deliveryRoots(descriptor, options.home, options.platform),
          options.platform,
          port,
        ))
      ) {
