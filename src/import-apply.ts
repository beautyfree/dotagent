...
type ImportJournalOperation = {
  operation: ImportOperation;
  status: "pending" | "staged" | "committing" | "applied";
};

type ImportJournal = {
  schemaVersion: typeof IMPORT_JOURNAL_VERSION;
  planId: string;
  library: string;
  phase: "applying" | "rolling-back";
  baseManifestText: string;
  baseConfigText: string;
  nextManifestText: string;
  nextConfigText: string;
  operations: ImportJournalOperation[];
};
...
async function writeJournal(journal: ImportJournal): Promise<void> {
...
async function assertOperationSource(operation: ImportOperation): Promise<void> {
  if (
    (operation.action !== "copy-owned" && operation.action !== "copy-vendored" && operation.action !== "adopt-owned") ||
    !operation.source ||
...
async function readImportJournal(library: string): Promise<ImportJournal | null> {
  try {
    const journal = JSON.parse(await readFile(journalPath(library), "utf8")) as ImportJournal;
...
export async function inspectImportRecovery(libraryRoot: string): Promise<ImportRecoveryPreview | null> {
...
export async function recoverImport(libraryRoot: string): Promise<"none" | "completed" | "rolled-back"> {
  const library = path.resolve(libraryRoot);
  const journal = await readImportJournal(library);
...
export async function applyImportPlan(plan: ImportPlan, options: ApplyImportOptions = {}): Promise<ApplyImportResult> {
  ...
  const copied = plan.operations.filter(
