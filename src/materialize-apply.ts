...
type MaterializationJournal = {
  schemaVersion: typeof MATERIALIZATION_JOURNAL_VERSION;
  planId: string;
  phase: "applying" | "rolling-back";
  previousState: MaterializationState;
...
async function sourceIntegrity(operation: MaterializationOperation): Promise<string> {
...
async function copyDirectory(source: string, destination: string): Promise<void> {
  ...
  for (const entry of await readdir(source, { withFileTypes: true })) {
    if (entry.name === COPY_MARKER || entry.name === ".git" || entry.name === "node_modules") continue;
    const from = path.join(source, entry.name);
    const to = path.join(destination, entry.name);
    if (entry.isSymbolicLink()) throw new Error(`Refusing linked source during copy: ${from}`);
...
async function validatePrecondition(operation: MaterializationOperation, state: MaterializationState): Promise<void> {
  ...
  if (operation.expectedTarget.state === "absent") {
    if (present) throw new Error(`Target appeared after review: ${operation.target}`);
    return;
  }
  if (!present) throw new Error(`Target disappeared after review: ${operation.target}`);
  if (operation.expectedTarget.state === "unmanaged") throw new Error(`Refusing unmanaged target: ${operation.target}`);
...
async function applyOperation(operation: MaterializationOperation, planId: string): Promise<void> {
  if (!operation.target) return;
  ...
  if (operation.action === "create-copy" || operation.action === "update-copy") {
    ...
    if (operation.action === "update-copy") await rename(operation.target, backupPath(operation, planId));
    await rename(stage, operation.target);
  }
}

async function rollbackJournal(libraryRoot: string, journal: MaterializationJournal): Promise<void> {
