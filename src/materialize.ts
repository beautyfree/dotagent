...
export function planMaterialization(
  ...
  targets: AgentMaterializationTarget[],
): MaterializationPlan {
  const operations: MaterializationOperation[] = [];
  const sortedTargets = [...targets].sort((left, right) =>
    left.descriptor.slug.localeCompare(right.descriptor.slug, "en"),
  );
  for (const target of sortedTargets) {
    ...
    for (const skill of inventory.ownedSkills) {
      ...
      if (target.mode === "native") {
        operations.push({
          agent: target.descriptor.slug,
          skill: skill.name,
          action: "available-native",
          source,
          sourceIntegrity: skill.integrity,
          target: null,
          expectedTarget: { state: "absent" },
        });
        continue;
      }
      if (!targetRoot) throw new Error(`Agent ${target.descriptor.slug} requires a resolved target root`);
      ...
      if (existing.state === "unmanaged") {
        ...
        const action: MaterializationAction =
        ...
        operations.push({
          ...
          ...(action === "conflict"
            ? {
                reason:
                  existing.state === "managed-copy" ? "Managed copy has local changes" : "Target is not a managed copy",
              }
            : {}),
        });
      } else {
