...
async function gitHead(root: string, git: WorkspaceGitPort): Promise<string | null> {
  try { return await git.run(["rev-parse", "--verify", "HEAD"], root); }
  catch { return null; }
}

async function gitBranch(root: string, git: WorkspaceGitPort): Promise<string> {
  try { return await git.run(["symbolic-ref", "--short", "HEAD"], root); }
...
function parseChangedPaths(output: string): string[] {
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    if (/[RC]/.test(status) && records[index + 1]) paths.add(records[++index]!.replaceAll("\\", "/"));
  }
...
async function changedLibraryPaths(root: string, git: WorkspaceGitPort): Promise<{ changed: string[]; ignored: string[] }> {
...
async function snapshotFiles(root: string, files: string[]): Promise<{
  ...
  for (const input of files) {
    ...
    if (!portable) { unsafePaths.push(input); continue; }
    const absolute = path.join(root, ...portable.split("/"));
    if (!await exists(absolute)) { snapshots.push({ path: portable, hash: null }); continue; }
    const metadata = await lstat(absolute);
    if (metadata.isSymbolicLink() || !metadata.isFile() || metadata.size > 10 * 1024 * 1024) { unsafePaths.push(portable); continue; }
    totalBytes += metadata.size;
    if (totalBytes > 100 * 1024 * 1024) { unsafePaths.push(portable); continue; }
    ...
    if (!content.subarray(0, Math.min(content.length, 8_192)).includes(0)) {
      for (const finding of scanTextForSecrets(content.toString("utf8"))) secretFindings.push({ file: portable, ...finding });
    }
  }
...
export async function initializeLibraryGit(root: string, remote?: string, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<void> {
...
export async function setLibraryRemote(root: string, remote: string, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<void> {
  const library = await ensureLibrary(root);
  normalizeGitIdentity(remote);
  try { await git.run(["remote", "get-url", "origin"], library); await git.run(["remote", "set-url", "origin", remote], library); }
  catch { await git.run(["remote", "add", "origin", remote], library); }
}

export async function cloneLibrary(remote: string, target: string, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<void> {
  normalizeGitIdentity(remote);
...
export async function getLibraryGitStatus(root: string, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<GitWorkspaceStatus> {
  const library = await ensureLibrary(root);
  ...
  try { remoteIdentity = normalizeGitIdentity(await git.run(["remote", "get-url", "origin"], library)); } catch { /* no remote */ }
  let ahead = 0;
  let behind = 0;
  try {
...
export async function planLibraryCommit(
  ...
  if (!commitMessage || commitMessage.length > 200 || /[\r\n\0]/.test(commitMessage)) throw new Error("Commit message must be one line between 1 and 200 characters");
  ...
  const payload = {
    kind: "git-commit" as const, schemaVersion: 1 as const, library, visibility, message: commitMessage, baseHead,
...
export async function applyLibraryCommit(plan: GitCommitPlan, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<string | null> {
  assertPlanId(plan);
...
export async function planLibraryPull(
  ...
  try {
    ...
    try { await git.run(["worktree", "remove", "--force", worktree], library); } catch { await rm(worktree, { recursive: true, force: true }); }
    await rm(worktreeParent, { recursive: true, force: true });
  }
  const payload = {
    kind: "git-pull" as const, schemaVersion: 1 as const, library, visibility, branch, baseHead: before.head, remoteHead, files,
...
export async function applyLibraryPull(plan: GitPullPlan, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<string> {
  assertPlanId(plan);
...
export async function planLibraryPush(root: string, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<GitPushPlan> {
  const library = await ensureLibrary(root);
  ...
  const payload = { kind: "git-push" as const, schemaVersion: 1 as const, library, branch: status.branch || DEFAULT_BRANCH, head: status.head, remoteIdentity: status.remoteIdentity, ahead: status.ahead };
  return withPlanId(payload);
}

export async function applyLibraryPush(plan: GitPushPlan, git: WorkspaceGitPort = new NodeWorkspaceGitPort()): Promise<void> {
  assertPlanId(plan);
