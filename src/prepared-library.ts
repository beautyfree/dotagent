 ...
 */
export async function prepareMaterializationInventory(
  options: PrepareMaterializationInventoryOptions,
  ...
  for (const dependency of prepared) {
    for (const lockedSkill of dependency.skills) {
      ...
      if (previous)
        throw new Error(
          `Skill name collision: ${scanned.value.name} is exported by ${previous} and dependency ${dependency.dependency}`,
        );
