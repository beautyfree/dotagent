 ...
 */
export async function prepareMaterializationInventory(options: PrepareMaterializationInventoryOptions): Promise<LibraryInventory> {
  ...
  if (dependencies.length === 0) return inventory.value;
  if (!loaded.value.lock) throw new Error("Dependencies require a reviewed skills.lock before materialization");
  ...
  const prepared = await Promise.all(dependencies.map(async ([name, dependency]) => {
    ...
    if (!locked) throw new Error(`skills.lock has no entry for dependency ${name}`);
  ...
  for (const dependency of prepared) {
    for (const lockedSkill of dependency.skills) {
      ...
      if (!scanned.ok) throw new DotagentError(`Prepared dependency ${dependency.dependency} is invalid`, scanned.issues);
      if (scanned.value.name !== lockedSkill.name) throw new Error(`Prepared dependency ${dependency.dependency} changed exported skill name`);
      const folded = scanned.value.name.toLocaleLowerCase("en-US");
      const previous = names.get(folded);
      if (previous) throw new Error(`Skill name collision: ${scanned.value.name} is exported by ${previous} and dependency ${dependency.dependency}`);
