...
> Agent Skills discussion #210 is still a proposal. dotagents exposes only the subset below and versions its own schemas independently.

dotagents follows the proposal's decentralized Git-package direction while keeping a richer internal model for safe desktop synchronization.
...
skills.lock
dotagents.yaml
dotagents.local.yaml  # gitignored
skills/
```
...
| Proposal concept | dotagents behavior | Compatibility status |
| --- | --- | --- |
...
| Final RFC filenames and version solver | dotagents owns versioned `skills.json` and `skills.lock` schemas until the proposal stabilizes | Intentional deviation |

## Safety extensions

dotagents adds behavior that a package-format RFC alone does not define:

- machine-local agent selection stays in gitignored `dotagents.local.yaml`;
