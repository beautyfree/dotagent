...
class FakeMachine implements MachinePort {
  ...
  async pathKind(filePath: string): Promise<MachinePathKind> {
    return this.kinds.get(filePath) ?? "missing";
  }
...
const codex: AgentDescriptor = {
  ...
  skills: [{ kind: "native-shared" }, { kind: "per-skill-link", roots: ["~/.codex/skills"] }],
};

describe("machine agent scan", () => {
  it("prefers command evidence", async () => {
    const inventory = await scanMachineAgents([codex], {
      platform: "darwin",
      home: "/home/test",
      port: new FakeMachine(new Set(["codex"])),
    });
  ...
  it("does not treat a directory containing only managed skills as an installed agent", async () => {
    const port = new FakeMachine(
      ...
      new Map([["/home/test/.codex", ["skills"]]]),
    );
  ...
  it("keeps unsupported platforms explicit", async () => {
    ...
    const inventory = await scanMachineAgents([descriptor], {
      platform: "win32",
      home: "C:\\Users\\test",
      port: new FakeMachine(),
    });
