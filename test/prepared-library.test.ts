...
describe("prepared dependency library", () => {
  it("materializes owned and locked dependency skills through the same safe plan", async () => {
    ...
    writeFileSync(join(source, "skills.json"), JSON.stringify({ schema_version: 1, name: "review-source", version: "1.0.0", skills: ["skills/review"], dependencies: {} }));
    execFileSync("git", ["add", "."], { cwd: source });
    execFileSync("git", ["commit", "-m", "source"], { cwd: source });
    const dependency = { url: pathToFileURL(source).href, ref: "HEAD" };
    const manifest = { schema_version: 1 as const, name: "portable-library", version: "0.1.0", skills: [], dependencies: { review: dependency } };
    ...
    const plan = planMaterialization(inventory, [{
      descriptor: { slug: "fixture", displayName: "Fixture", platforms: [process.platform as "darwin" | "linux" | "win32"], detection: [], skills: [{ kind: "copy-only", roots: [target] }] },
