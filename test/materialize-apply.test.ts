...
const copyAgent: AgentDescriptor = {
  ...linkAgent,
  slug: "copy-agent",
  skills: [{ kind: "copy-only", roots: ["unused"] }],
};
...
describe("materialization apply", () => {
  it("creates a reviewed managed link and records local ownership", async () => {
    ...
    expect((await readMaterializationState(library)).targets[target]).toMatchObject({
      agent: "test-agent",
      skill: "writing",
    });
  });
  ...
  it("rolls back earlier writes when a later operation fails", async () => {
    const { library, targetRoot, inventory } = await fixture();
    const firstSkill = inventory.ownedSkills[0];
    if (!firstSkill) throw new Error("fixture has no owned skill");
    const duplicated = {
      ...inventory,
      ownedSkills: [...inventory.ownedSkills, { ...firstSkill, name: "second", path: "skills/writing" }],
    };
    const plan = planMaterialization(duplicated, [
      {
  ...
  it("creates a copy with a marker that does not alter content integrity", async () => {
    const { targetRoot, inventory } = await fixture();
    const firstSkill = inventory.ownedSkills[0];
    if (!firstSkill) throw new Error("fixture has no owned skill");
    const plan = planMaterialization(inventory, [
      {
    ...
    expect(targetScan).toBe(firstSkill.integrity);
  });
  ...
  it("updates only a previously managed copy", async () => {
    ...
    const firstSkill = inventory.ownedSkills[0];
    if (!firstSkill) throw new Error("fixture has no owned skill");
    const initial = planMaterialization(inventory, [
      {
        descriptor: copyAgent,
        platform: process.platform as "darwin" | "linux" | "win32",
        detected: true,
    ...
    const update = planMaterialization(rescanned.value, [
      {
        ...
        existing: {
          writing: {
            state: "managed-copy",
            integrity: firstSkill.integrity,
            baseIntegrity: firstSkill.integrity,
          },
        },
      },
    ]);
  ...
  it("recovers an interrupted managed create from its durable journal", async () => {
    ...
    if (!operation?.target) throw new Error("fixture produced no target operation");
