...
export class SourceReleaseAgeError extends Error {
    ...
    constructor(decision) {
        super(`Source ${decision.source} resolved to a commit that is ${decision.ageMinutes} minutes old; ` +
...
export function parseSourceSecurityPolicy(input = {}) {
    ...
    return {
        trust: {
            ...
            github_organizations: uniqueSorted(parsed.trust.github_organizations.map((entry) => entry.toLocaleLowerCase("en-US"))),
...
export function exactSourceSecurityPolicy(sources, options = {}) {
...
function repositoryParts(identity) {
...
export function requireTrustedSource(source, input) {
...
export function requireMinimumReleaseAge(source, committedAt, input, now = new Date()) {
    const policy = parseSourceSecurityPolicy(input);
    const identity = normalizedRepository(source);
    const timestamp = Date.parse(committedAt);
    if (!Number.isFinite(timestamp))
        throw new Error(`Git returned an invalid commit timestamp for ${identity}`);
    ...
    if (!excluded && ageMinutes < policy.minimum_release_age_minutes)
        throw new SourceReleaseAgeError(decision);
    return decision;
}
