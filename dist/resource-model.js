...
import { z } from "zod";
import { scanSkillForSecrets, scanTextForSecrets } from "./audit.js";
import { scanOwnedSkill } from "./inventory.js";
import { normalizePortablePath } from "./paths.js";
import { computePlanId } from "./plan.js";
import { agentResourceCapabilities } from "./agents.js";
export const RESOURCE_MANIFEST_VERSION = 2;
export const RESOURCE_KINDS = ["skill", "instruction", "command", "subagent"];
...
export async function scanResourceManifest(root, input, maximumFileBytes = 1024 * 1024) {
    const manifest = resourceManifestSchema.parse(input);
    const library = path.resolve(root);
    const resources = [];
    for (const descriptor of [...manifest.resources].sort((left, right) => `${left.kind}:${left.id}`.localeCompare(`${right.kind}:${right.id}`, "en"))) {
        if (descriptor.kind === "skill") {
            const scanned = await scanOwnedSkill(library, descriptor.path);
            if (!scanned.ok)
                throw new Error(`Resource ${descriptor.kind}:${descriptor.id} failed safe skill scanning`);
            resources.push({
...
export function planResourceComposition(inputs) {
...
function capabilityFor(matrix, kind) {
    return matrix[kind];
}
/** Explain delivery support before any native agent file is touched. */
export function planResourceProjection(resources, agents) {
    const bySlug = new Map(agents.map((agent) => [agent.slug, agent]));
    const projections = [];
    for (const resource of [...resources].sort((left, right) => `${left.kind}:${left.id}`.localeCompare(`${right.kind}:${right.id}`, "en"))) {
