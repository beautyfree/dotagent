...
export const RESOURCE_PROJECTION_STATE_VERSION = 1;
export const RESOURCE_PROJECTION_JOURNAL_VERSION = 1;
...
function assertNotBroadRoot(root, label) {
    if (root === path.parse(root).root)
        throw new Error(`Refusing broad ${label}: ${root}`);
}
...
export function resourceProjectionStatePath(library) {
    return path.join(metadataRoot(path.resolve(library)), "resource-projection-state.json");
}
export function resourceProjectionJournalPath(library) {
...
export async function readResourceProjectionState(library) {
    try {
        const parsed = JSON.parse(await readFile(resourceProjectionStatePath(library), "utf8"));
        if (parsed.schemaVersion !== RESOURCE_PROJECTION_STATE_VERSION ||
            !parsed.targets ||
...
async function assertNoLinkedParents(root, relativePath) {
    let current = root;
...
function assertDistinctTargets(resources) {
 ...
 */
export async function planManagedResourceProjection(input) {
    ...
    if (librarySnapshot.state !== "directory")
        throw new Error(`Resource library does not exist: ${library}`);
    ...
    const operations = [];
    for (const resource of [...input.resources].sort((left, right) => left.targetPath.localeCompare(right.targetPath, "en"))) {
        ...
        else if (expectedOwnership.agent !== input.agent ||
...
async function assertPlanPreconditions(plan, state) {
...
async function rollbackResourceProjection(journal) {
...
export async function recoverManagedResourceProjection(library) {
    try {
        const parsed = JSON.parse(await readFile(resourceProjectionJournalPath(library), "utf8"));
        if (parsed.kind !== "managed-resource-projection" ||
...
export async function applyManagedResourceProjection(plan, options = {}) {
    const { planId, ...payload } = plan;
    if (computePlanId(payload) !== planId)
        throw new Error("Resource projection plan is stale or modified");
...
//# sourceMappingURL=resource-apply.js.map