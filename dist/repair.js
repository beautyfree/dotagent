...
function currentGitignore(root) {
    const file = path.join(path.resolve(root), ".gitignore");
    return existsSync(file) ? readFileSync(file, "utf8") : "";
}
function repairedGitignore(current) {
...
export function planDoctorRepair(report, selectedCodes) {
    const selected = new Set(selectedCodes);
    const present = new Set(report.issues.map((issue) => issue.code));
    for (const code of selected) {
        if (!present.has(code))
            throw new Error(`Selected repair finding is no longer present: ${code}`);
    }
    const actions = [];
    const unsupported = [];
...
export function applyDoctorRepair(root, plan, expectedPlanId) {
    ...
    if (plan.hasBlockers)
        throw new Error("Doctor Repair plan contains unsupported findings");
