...
import { NodeWorkspaceGitPort } from "./git-workspace.js";
...
function assertPlanId(plan) {
    ...
    if (computePlanId(payload) !== planId)
        throw new Error("Git fast-forward plan is stale or modified");
}
/**
 ...
 */
export async function planGitFastForward(root, sourcePolicy = {}, git = new NodeWorkspaceGitPort()) {
    ...
    if (topLevel !== workspace)
        throw new Error("Git fast-forward root must be the repository worktree root");
    const changed = await git.run(["status", "--porcelain=v1", "-z"], workspace, { raw: true });
    if (changed.length > 0)
        throw new Error("Git workspace has uncommitted changes; resolve them before review");
    const branch = await git.run(["symbolic-ref", "--quiet", "--short", "HEAD"], workspace);
    if (!branch)
        throw new Error("Git workspace must have a checked-out branch");
    ...
    const fastForward = await git.run(["merge-base", "--is-ancestor", baseHead, remoteHead], workspace).then(() => true, () => false);
    if (!fastForward)
        throw new Error("Remote history is not a fast-forward; reconcile it explicitly");
    const files = baseHead === remoteHead
        ? []
        : parseNullPaths(await git.run(["diff", "--name-only", "-z", `${baseHead}..${remoteHead}`], workspace, { raw: true }));
    const payload = {
...
export async function inspectGitFastForwardPlan(plan, inspect, git = new NodeWorkspaceGitPort()) {
    assertPlanId(plan);
    const current = await planGitFastForward(plan.workspace, plan.sourcePolicy, git);
    if (current.planId !== plan.planId)
        throw new Error("Remote or local Git state changed after review");
...
export async function applyGitFastForwardPlan(plan, git = new NodeWorkspaceGitPort()) {
    assertPlanId(plan);
    const current = await planGitFastForward(plan.workspace, plan.sourcePolicy, git);
    if (current.planId !== plan.planId)
        throw new Error("Remote or local Git state changed after review");
    if (plan.baseHead !== plan.remoteHead)
        await git.run(["merge", "--ff-only", plan.remoteHead], plan.workspace);
    return git.run(["rev-parse", "HEAD"], plan.workspace);
}
//# sourceMappingURL=git-fast-forward.js.map