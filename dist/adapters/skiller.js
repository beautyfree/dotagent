...
function withSkillerPublishPlanId(payload) {
    return { ...payload, planId: computePlanId(payload) };
}
export function assertSkillerStableId(id) {
    if (!skillerStableIdSchema.safeParse(id).success)
...
export function assertSkillerPortableRelativePath(value) {
...
export function assertSkillerPortableSkillSourcePath(value) {
    if (value !== ".")
        assertSkillerPortableRelativePath(value);
}
/** Credentials belong in a credential helper or SSH agent, never in a portable manifest. */
export function assertCredentialFreeGitRemote(remote) {
    if (/^[a-z][a-z0-9+.-]*:\/\/[^/\s@]+:[^/\s@]+@/i.test(remote)) {
        throw new Error("Sync repository URL must not embed credentials");
    }
}
export function validateSkillerSyncManifest(input) {
    const raw = input;
    ...
    for (const skill of manifest.skills) {
        if (seenIds.has(skill.id))
            throw new Error(`Duplicate sync skill id: ${skill.id}`);
...
export function parseSkillerSyncManifest(text) {
    let parsed;
    try {
        parsed = parse(text);
    }
...
export function stringifySkillerSyncManifest(manifest) {
    return stringify(validateSkillerSyncManifest(manifest));
}
export function createSkillerSyncManifest(profileId, mode = "private", agentPolicy = { mode: "detected" }) {
...
function normalizedInstallations(agentSlugs) {
...
export function planSkillerSyncPublish(profileId, mode, candidates, agentPolicy) {
    ...
    const bundledSkills = bundledCandidates.map((candidate) => {
    ...
    manifest.skills = candidates.map((candidate) => {
        ...
        if (!skill)
            throw new Error(`Missing bundled export plan: ${candidate.id}`);
        return {
            id: skill.id,
            kind: "bundled",
    ...
    return withSkillerPublishPlanId({
        ...
        bundledSkills,
        bundledDistributions: Object.fromEntries(bundledCandidates.map((candidate) => [candidate.id, candidate.kind === "vendored" ? "vendored" : "owned"])),
        vendoredOrigins: Object.fromEntries(bundledCandidates
            .filter((candidate) => candidate.kind === "vendored")
...
export function mergeSkillerSyncPublishUpdate(base, update, options = {}) {
...
//# sourceMappingURL=skiller.js.map