...
export function builtinAgentCatalogEntry(slug) {
    const entry = entries.find((candidate) => candidate.slug === slug);
    return entry ? cloneEntry(entry) : null;
}
export function agentCatalogEntryToDescriptor(entry, options = {}) {
    const sharedSkillsPath = options.sharedSkillsPath ?? "~/.agents/skills";
    const readsShared = entry.readableRoots.some((root) => root.sourceAgent === "shared" || root.path === sharedSkillsPath);
    const skills = [
        ...(readsShared ? [{ kind: "native-shared" }] : []),
        ...(entry.skillRoots.length > 0 ? [{ kind: "per-skill-link", roots: [...entry.skillRoots] }] : []),
    ];
    if (skills.length === 0)
        skills.push({ kind: "config-path", configId: `builtin:${entry.slug}` });
    return {
        slug: entry.slug,
        displayName: entry.displayName,
        platforms: options.platforms ?? ["darwin", "linux", "win32"],
        detection: [
            ...(entry.command ? [{ kind: "command", command: entry.command }] : []),
...
export function builtinAgentDescriptors(options = {}) {
